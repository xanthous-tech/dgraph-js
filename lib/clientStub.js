"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DgraphClientStub = void 0;
var grpc_1 = require("grpc");
var proto_loader_1 = require("@grpc/proto-loader");
var path_1 = require("path");
var api_1 = require("../generated/api");
var errors_1 = require("./errors");
var SERVICE_NAME = 'Dgraph';
var DgraphClientStub = (function () {
    function DgraphClientStub(addr, credentials, options) {
        this.accessJwt = "";
        this.refreshJwt = "";
        if (addr === undefined) {
            addr = "localhost:9080";
        }
        if (credentials === undefined) {
            credentials = grpc_1.default.credentials.createInsecure();
        }
        var serviceDefinition = proto_loader_1.loadSync(path_1.resolve(__dirname, '..', 'protos', 'api.proto'))[SERVICE_NAME];
        var Client = grpc_1.default.makeGenericClientConstructor(serviceDefinition, SERVICE_NAME, {});
        this.stub = new Client(addr, credentials, options);
    }
    DgraphClientStub.prototype.login = function (userid, password, refreshJwt, metadata, options) {
        return __awaiter(this, void 0, void 0, function () {
            var req, jwtResponse;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        req = {};
                        if (userid !== undefined) {
                            req.userid = userid;
                            req.password = password;
                        }
                        else if (refreshJwt !== undefined) {
                            req.refreshToken = refreshJwt;
                        }
                        else {
                            req.refreshToken = this.refreshJwt;
                        }
                        return [4, new Promise(function (resolve, reject) {
                                _this.stub.login(new api_1.api.LoginRequest(req), _this.ensureMetadata(metadata), ensureCallOptions(options), function (error, payload) {
                                    if (error) {
                                        reject(error);
                                        return;
                                    }
                                    resolve(payload);
                                });
                            })];
                    case 1:
                        jwtResponse = _a.sent();
                        this.accessJwt = jwtResponse.accessJwt;
                        this.refreshJwt = jwtResponse.refreshJwt;
                        return [2, jwtResponse];
                }
            });
        });
    };
    DgraphClientStub.prototype.alter = function (op, metadata, options) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.stub.alter(op, _this.ensureMetadata(metadata), ensureCallOptions(options), function (error, payload) {
                if (error) {
                    reject(error);
                    return;
                }
                resolve(payload);
            });
        });
    };
    DgraphClientStub.prototype.retryLogin = function (metadata, options) {
        return __awaiter(this, void 0, void 0, function () {
            var req, jwtResponse;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (this.refreshJwt.length === 0) {
                            throw errors_1.ERR_REFRESH_JWT_EMPTY;
                        }
                        req = {};
                        req.refreshToken = this.refreshJwt;
                        return [4, new Promise(function (resolve, reject) {
                                _this.stub.login(new api_1.api.LoginRequest(req), _this.ensureMetadata(metadata), ensureCallOptions(options), function (error, jwt) {
                                    if (error) {
                                        reject(error);
                                        return;
                                    }
                                    resolve(jwt);
                                });
                            })];
                    case 1:
                        jwtResponse = _a.sent();
                        this.accessJwt = jwtResponse.accessJwt;
                        this.refreshJwt = jwtResponse.refreshJwt;
                        return [2, jwtResponse];
                }
            });
        });
    };
    DgraphClientStub.prototype.query = function (req, metadata, options) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.stub.query(req, _this.ensureMetadata(metadata), ensureCallOptions(options), function (error, response) {
                if (error) {
                    reject(error);
                    return;
                }
                resolve(response);
            });
        });
    };
    DgraphClientStub.prototype.mutate = function (mu, metadata, options) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.stub.mutate(mu, _this.ensureMetadata(metadata), ensureCallOptions(options), function (error, response) {
                if (error) {
                    reject(error);
                    return;
                }
                resolve(response);
            });
        });
    };
    DgraphClientStub.prototype.commitOrAbort = function (ctx, metadata, options) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.stub.commitOrAbort(ctx, _this.ensureMetadata(metadata), ensureCallOptions(options), function (error, txnContext) {
                if (error) {
                    reject(error);
                    return;
                }
                resolve(txnContext);
            });
        });
    };
    DgraphClientStub.prototype.checkVersion = function (check, metadata, options) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.stub.mutate(check, _this.ensureMetadata(metadata), ensureCallOptions(options), function (error, version) {
                if (error) {
                    reject(error);
                    return;
                }
                resolve(version);
            });
        });
    };
    DgraphClientStub.prototype.waitForReady = function (deadline) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.stub.waitForReady(deadline, function (error) {
                if (error) {
                    reject(error);
                    return;
                }
                resolve();
            });
        });
    };
    DgraphClientStub.prototype.close = function () {
        return this.stub.close();
    };
    DgraphClientStub.prototype.grpcClient = function () {
        return this.stub;
    };
    DgraphClientStub.prototype.ensureMetadata = function (metadata) {
        var newMeta = metadata === undefined ? new grpc_1.default.Metadata() : metadata;
        if (this.accessJwt !== "") {
            newMeta.set("accessJwt", this.accessJwt);
        }
        return newMeta;
    };
    return DgraphClientStub;
}());
exports.DgraphClientStub = DgraphClientStub;
function ensureCallOptions(options) {
    return options === undefined
        ? {
            propagate_flags: grpc_1.default.propagate.DEFAULTS,
            credentials: undefined,
        }
        : options;
}
